...
class ChangeDetection:
    ...
    def processAllDocuments(self,table_name,ids):
       ...
       if len(ids):
           id_value = ','.join(ids)
           stmt = 'select id,external_link_address from %s where id in(%s) order by id asc' %(table_name,id_value)
       else:
           stmt = 'select id,external_link_address from %s order by id asc' %(table_name)
       cursor.execute(stmt)
